	...
	var blocksForColonyInvasion = function(info) {
		...
		if (info.colony.embassy == 1) {
			...
			opts = [
				...
				{ key: "BFCI-ASS3", text: "%X %L"+event.oldSize+" people were evacuated from the %U system as invasion looked likely. In the end the system was spared, but so far only the orbital stations have been reactivated.", condition: info.economy.type != "Quarantine" },
	...
	var blocksForRegionalInfo = function(info) {
		...
		if (info.s == region.influential[0]) {
			...
			opts = [
				...
				{ key: "BFRI-RCAP35", text: "The rulers of %H have wider authority over the whole "+region.name+" area.", condition: region.category.match(/Alliance/) && region.subCategory == info.politics.governmentCategory && region.subCategory == "Hierarchical" },
