...
this.updateGeneralCommodityDefinition = function(marketdef, station, system) {
	...
	var priceband = 0;
	for (var i=1;i<=22;i++) {
		if (classes.indexOf("sotl-priceband-"+i) != -1) {
			priceband = i;
			break;
		}
	}
	...
	if (classes.indexOf("sotl-volatility-low") != -1) {
		volatility = 0.2; rolls = 4;
	} else if (classes.indexOf("sotl-volatility-med") != -1) {
		volatility = 0.4; rolls = 3;
	} else if (classes.indexOf("sotl-volatility-high") != -1) {
		volatility = 0.6; rolls = 2;
	} else if (classes.indexOf("sotl-volatility-extreme") != -1) {
	...
	// economy)
	var security = sysinfo.sotl_system_stability;
	...
	var ecreason = sysinfo.sotl_economy_reason;
	var ecstatus = parseInt(sysinfo.sotl_economy_status);
	...
	if (classes.indexOf("sotl-im-"+eckey) != -1 && classes.indexOf("sotl-ex-"+eckey) != -1) {
	...
	} else if (classes.indexOf("sotl-im-"+eckey) != -1) {
		priceband += tvolfactor;
		imported = true;
	} else if (classes.indexOf("sotl-ex-"+eckey) != -1) {
		priceband -= tvolfactor;
		exported = true;
	} else {
		// neither import nor export but there may be special factors
		if (classes.indexOf("sotl-imr-"+ecrkey) != -1) {
			priceband += tvolfactor/2;
			imported = true;
		} else if (classes.indexOf("sotl-exr-"+ecrkey) != -1) {
			priceband -= tvolfactor/2;
			exported = true;
		} else {
			...
			if (classes.indexOf("sotl-ims-radiation") != -1 && parseFloat(sysinfo.sotl_planet_surface_radiation) > 0.3) {
				// high radiation imports
				priceband += tvolfactor;
				imported = true;
			} else if (classes.indexOf("sotl-ims-cold") != -1 && parseFloat(sysinfo.sotl_planet_surface_temperature) < 5) {
				// low temperature imports
				priceband += tvolfactor;
				imported = true;
			} else if (classes.indexOf("sotl-exs-ocean") != -1 && parseFloat(sysinfo.sotl_habitability_best) > 80 && parseFloat(sysinfo.percent_land) < 30) {
				// habitable ocean worlds
				priceband -= tvolfactor/2;
				exported = true;
			} else if (classes.indexOf("sotl-ims-unstable") != -1 && (security <= 2 || sysinfo.sotl_government_category == "Disordered")) {
				// unstable governments
				priceband += tvolfactor/2;
				imported = true;
			} else if (classes.indexOf("sotl-ims-nohab") != -1 && parseFloat(sysinfo.sotl_habitability_best) < 60) {
				// uninhabitable worlds
				priceband += tvolfactor/2;
				imported = true;
			} else if (classes.indexOf("sotl-exs-oilmining") != -1 && parseFloat(sysinfo.sotl_habitability_best) > 60 && eckey == "groundmining") {
				// oil-rich worlds
				priceband -= tvolfactor;
				exported = true;
			} else if (classes.indexOf("sotl-exs-hab") != -1 && parseFloat(sysinfo.sotl_habitability_best) > 60) {
				// inhabitable worlds
				priceband -= tvolfactor/2;
				exported = true;
			} else if (classes.indexOf("sotl-exs-minrich") != -1 && parseFloat(sysinfo.sotl_mineral_wealth) > 0.45) {
				// mineral rich worlds
				priceband -= tvolfactor/2;
				exported = true;
			} else if (classes.indexOf("sotl-exs-tech") != -1 && sysinfo.techlevel > 8) {
				// inhabitable worlds
				priceband -= tvolfactor/2;
				exported = true;
			} else if (classes.indexOf("sotl-ims-isolation") != -1 && sysinfo.government_description == "Isolationist") {
				// inhabitable worlds
				priceband += tvolfactor;
				exported = true;


			} else if (classes.indexOf("sotl-ims-rich") != -1 && productivity > 1E6) {
	...
	if (classes.indexOf("sotl-demand-high") != -1 && imported) {
		pzeroes += 2;
		tvolfactor = -2;
	} else if (classes.indexOf("sotl-supply-high") != -1 && exported) {
		pzeroes += 2;
		tvolfactor = 2;
	} else if (classes.indexOf("sotl-demand-medium") != -1 && imported) {
		pzeroes += 1;
		tvolfactor = -1;
	} else if (classes.indexOf("sotl-supply-medium") != -1 && exported) {
		pzeroes += 1;
		tvolfactor = 1;
	} else if (classes.indexOf("sotl-demand-low") != -1 && imported) {
		pzeroes -= 0;
		tvolfactor = -0.5;
	} else if (classes.indexOf("sotl-supply-low") != -1 && exported) {
		pzeroes -= 0;
		tvolfactor = 0.5;
	} else if (classes.indexOf("sotl-supply-none") != -1) {
		pzeroes += 0; // no change
		tvolfactor = -100;
	} else {
		// neither import nor export
		if (classes.indexOf("sotl-specialist-good")) {
			pzeroes -= 1;
	...
	if (classes.indexOf("sotl-quantity-bulk") != -1) {
		pzeroes += 2;
		if (pzeroes < 4) {
			pzeroes = 4;
		}
		tvolfactor += 0.5; // available almost anywhere
	} else if (classes.indexOf("sotl-quantity-high") != -1) {
		pzeroes += 1;
		tvolfactor += 0.25; // available almost anywhere
	} else if (classes.indexOf("sotl-quantity-medium") != -1) {
		pzeroes += 0;
	} else if (classes.indexOf("sotl-quantity-low") != -1) {
		pzeroes -= 1;
	} else if (classes.indexOf("sotl-quantity-rare") != -1) {
		pzeroes -= 2;
	}
